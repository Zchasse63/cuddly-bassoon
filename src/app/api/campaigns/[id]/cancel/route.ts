...
export async function POST(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const success = await campaignService.cancelCampaign(user.id, id);

    if (!success) {
      return NextResponse.json({ error: 'Failed to cancel campaign' }, { status: 400 });
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('[API] Campaign cancel error:', error);
