...
const searchPropertiesHandler: ToolHandler<SearchPropertiesInput, SearchPropertiesOutput> = async (
  input,
  _context
) => {
  ...
  try {
    ...
    try {
      ...
      if (input.location) {
        query.or(
          `city.ilike.%${input.location}%,state.ilike.%${input.location}%,zip.eq.${input.location}`
        );
      }
      ...
      const {
        ...
        count,
        error: dbError,
      } = await query.limit(input.limit).range(input.offset, input.offset + input.limit - 1);
...
const getPropertyDetailsDefinition: ToolDefinition<
...
const getPropertyDetailsHandler: ToolHandler<
  GetPropertyDetailsInput,
  GetPropertyDetailsOutput
> = async (input, _context) => {
  ...
  try {
    ...
    try {
      ...
      console.error('[Property Search] Fallback error:', fallbackError);
      throw new Error(
        `Property details failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
};
