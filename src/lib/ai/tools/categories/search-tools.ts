...
const searchByDescDef: ToolDefinition<SearchByDescInput, SearchByDescOutput> = {
  ...
  name: 'searchPropertiesByDescription',
  description: 'Search for properties using natural language description',
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: searchByDescInput,
  outputSchema: searchByDescOutput,
  requiresConfirmation: false,
  tags: ['search', 'natural-language'],
};

const searchByDescHandler: ToolHandler<SearchByDescInput, SearchByDescOutput> = async (
  input,
  _ctx
) => {
...
const executeFilterOutput = z.object({
  properties: z.array(
    z.object({ id: z.string(), address: z.string(), city: z.string().nullable() })
  ),
...
const executeFilterDef: ToolDefinition<ExecuteFilterInput, ExecuteFilterOutput> = {
  ...
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: executeFilterInput,
  outputSchema: executeFilterOutput,
  requiresConfirmation: false,
  tags: ['search', 'filter'],
};

const executeFilterHandler: ToolHandler<ExecuteFilterInput, ExecuteFilterOutput> = async (
  input,
  ctx
) => {
  const supabase = await createClient();
...
const saveSearchOutput = z.object({
  filterId: z.string(),
  filterName: z.string(),
  message: z.string(),
});
...
const saveSearchDef: ToolDefinition<SaveSearchInput, SaveSearchOutput> = {
  ...
  name: 'saveSearchAsFilter',
  description: 'Save search criteria as a reusable filter',
  category: 'property_search',
...
const getRecentDef: ToolDefinition<GetRecentInput, GetRecentOutput> = {
  ...
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: getRecentInput,
  outputSchema: getRecentOutput,
  requiresConfirmation: false,
  tags: ['search', 'history'],
};
...
const getRecentHandler: ToolHandler<GetRecentInput, GetRecentOutput> = async (input, ctx) => {
  ...
  const searches = (data || []).map((s) => ({
    ...
    resultsCount: s.results_count || 0,
...
const refineDef: ToolDefinition<RefineInput, RefineOutput> = {
  ...
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: refineInput,
  outputSchema: refineOutput,
  requiresConfirmation: false,
  tags: ['search', 'refine'],
};
...
const compareInput = z.object({
  searchA: z.array(z.string()),
  searchB: z.array(z.string()),
});
...
const compareHandler: ToolHandler<CompareInput, CompareOutput> = async (input) => {
  ...
  return {
    ...
    onlyInB,
    overlapPct,
    summary: `${overlap.length} in common (${overlapPct.toFixed(1)}%)`,
  };
};
...
const exportHandler: ToolHandler<ExportInput, ExportOutput> = async (input, ctx) => {
  ...
  if (input.format === 'json') {
    ...
    const rows = (data || []).map((p) =>
      [p.address, p.city, p.state, p.zip, p.bedrooms, p.bathrooms].join(',')
    );
...
const scheduleInput = z.object({
  ...
  notifyVia: z.enum(['email', 'sms', 'both', 'none']).default('email'),
});
...
const scheduleHandler: ToolHandler<ScheduleInput, ScheduleOutput> = async (input, ctx) => {
  ...
  return {
    ...
    message: `Search scheduled ${input.frequency}`,
  };
};
...
const suggestHandler: ToolHandler<SuggestInput, SuggestOutput> = async (input, ctx) => {
  ...
  if (deals && deals.length > 0) {
    suggestions.push({
      ...
      reason: `Based on your ${deals.length} successful deals`,
      confidence: 80,
    });
  }
  suggestions.push({
    criteria: { ownerType: 'absentee', ...(input.currentCriteria || {}) },
    reason: 'Absentee owners are often more motivated',
    confidence: 75,
  });
  return { suggestions };
};
...
const analyzeHandler: ToolHandler<AnalyzeInput, AnalyzeOutput> = async (input, ctx) => {
  ...
  if (totalSearches < 10)
    recommendations.push('Increase search frequency to find more opportunities');
