...
const valuationOutput = z.object({
  ...
  breakdown: z.object({
    baseValue: z.number(),
    adjustments: z.array(z.object({ factor: z.string(), amount: z.number() })),
  }),
});
...
const valuationDef: ToolDefinition<ValuationInput, ValuationOutput> = {
  ...
  inputSchema: valuationInput,
  outputSchema: valuationOutput,
  requiresConfirmation: false,
  tags: ['property', 'valuation', 'arv'],
};
...
const valuationHandler: ToolHandler<ValuationInput, ValuationOutput> = async (input, ctx) => {
  ...
  const adjustments = [
    ...
    { factor: 'Bathrooms', amount: ((property.bathrooms || 2) - 2) * 8000 },
    {
      factor: 'Condition',
      amount:
        property.condition === 'excellent' ? 20000 : property.condition === 'poor' ? -30000 : 0,
    },
  ];
  ...
  return {
    ...
    arv,
    confidence: 75,
    breakdown: { baseValue, adjustments },
  };
};
...
const compsInput = z.object({
  propertyId: z.string(),
  radiusMiles: z.number().optional().default(1),
...
const compsDef: ToolDefinition<CompsInput, CompsOutput> = {
  ...
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: compsInput,
  outputSchema: compsOutput,
  requiresConfirmation: false,
  tags: ['property', 'comps', 'analysis'],
};
...
const compsHandler: ToolHandler<CompsInput, CompsOutput> = async (input, ctx) => {
  ...
  const { data: comps } = await supabase
    ...
    .neq('id', input.propertyId)
    .limit(input.limit);

  const comparables = (comps || []).map((c, idx) => ({
    id: c.id,
    address: c.address,
    salePrice: c.asking_price,
    sqft: c.square_footage,
    adjustedValue: c.asking_price || 200000,
    similarity: 90 - idx * 5,
...
const motivationDef: ToolDefinition<MotivationInput, MotivationOutput> = {
  ...
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: motivationInput,
  outputSchema: motivationOutput,
  requiresConfirmation: false,
  tags: ['property', 'motivation', 'analysis'],
};
...
const motivationHandler: ToolHandler<MotivationInput, MotivationOutput> = async (input, ctx) => {
  ...
  if (property.owner_type === 'absentee') {
    score += 15;
    factors.push({ factor: 'Absentee Owner', impact: 15, description: 'Owner lives elsewhere' });
  }
  if (property.condition === 'poor' || property.condition === 'distressed') {
    ...
    factors.push({
      ...
      impact: 20,
      description: 'Property needs significant work',
    });
  }
...
const summaryInput = z.object({
  propertyId: z.string(),
  audience: z.enum(['investor', 'buyer', 'agent']).default('investor'),
});
...
const dealOutput = z.object({
  ...
  metrics: z.object({ arv: z.number(), repairs: z.number(), maxOffer: z.number() }),
});
...
const dealHandler: ToolHandler<DealInput, DealOutput> = async (input, ctx) => {
  ...
  const repairs =
    property.condition === 'poor' ? 50000 : property.condition === 'fair' ? 25000 : 10000;
  ...
  const recommendation =
    dealScore >= 70
      ? 'Strong deal potential - recommend pursuing'
      : 'Marginal deal - proceed with caution';
...
const ownershipDef: ToolDefinition<OwnershipInput, OwnershipOutput> = {
  ...
  outputSchema: ownershipOutput,
  requiresConfirmation: false,
  tags: ['property', 'ownership', 'history'],
};
...
const ownershipHandler: ToolHandler<OwnershipInput, OwnershipOutput> = async (input, ctx) => {
  ...
  const history = [
    { owner: 'Previous Owner', from: '2015-01-01', to: '2019-01-01', salePrice: 180000 },
  ];
  const patterns =
    durationYears > 10
      ? ['Long-term owner - may be less motivated']
      : ['Recent purchase - standard ownership'];
...
const repairOutput = z.object({
  ...
  breakdown: z.array(
    z.object({ category: z.string(), low: z.number(), mid: z.number(), high: z.number() })
  ),
});
  ...
  const breakdown = [
    {
      category: 'Kitchen',
      low: 5000 * multiplier,
      mid: 10000 * multiplier,
      high: 20000 * multiplier,
    },
    {
      category: 'Bathrooms',
      low: 3000 * multiplier,
      mid: 6000 * multiplier,
      high: 12000 * multiplier,
    },
    {
      category: 'Flooring',
      low: 2000 * multiplier,
...
const tomDef: ToolDefinition<TomInput, TomOutput> = {
  ...
  name: 'predictTimeOnMarket',
  description: 'Predict days to sell based on market and property characteristics',
  category: 'property_search',
  requiredPermission: 'read',
  inputSchema: tomInput,
  outputSchema: tomOutput,
  requiresConfirmation: false,
  tags: ['property', 'market', 'prediction'],
};
...
const tomHandler: ToolHandler<TomInput, TomOutput> = async (input, ctx) => {
  ...
  if (property.condition === 'excellent') {
    predictedDays -= 10;
    factors.push({ factor: 'Excellent condition', impact: '-10 days' });
  } else if (property.condition === 'poor') {
    predictedDays += 20;
    factors.push({ factor: 'Poor condition', impact: '+20 days' });
  }
  if ((property.asking_price || 0) < 200000) {
    predictedDays -= 5;
    factors.push({ factor: 'Lower price point', impact: '-5 days' });
  }
...
const neighborhoodOutput = z.object({
  overview: z.string(),
  metrics: z.object({ crimeIndex: z.number(), schoolRating: z.number(), walkScore: z.number() }),
...
const neighborhoodDef: ToolDefinition<NeighborhoodInput, NeighborhoodOutput> = {
  ...
  requiresConfirmation: false,
  tags: ['property', 'neighborhood', 'analysis'],
};

const neighborhoodHandler: ToolHandler<NeighborhoodInput, NeighborhoodOutput> = async (
  input,
  ctx
) => {
  const supabase = await createClient();
...
const portfolioDef: ToolDefinition<PortfolioInput, PortfolioOutput> = {
  ...
  requiresConfirmation: false,
  tags: ['property', 'portfolio', 'comparison'],
};
...
const portfolioHandler: ToolHandler<PortfolioInput, PortfolioOutput> = async (input, ctx) => {
  ...
  const similarDeals = (deals || []).map((d, idx) => ({
    dealId: d.id,
    address: d.property_address || 'Unknown',
    similarity: 85 - idx * 5,
    outcome: 'Profitable',
  }));
...
const offerInput = z.object({
  propertyId: z.string(),
  targetProfit: z.number().optional().default(20000),
});
...
const rentalHandler: ToolHandler<RentalInput, RentalOutput> = async (input, ctx) => {
  ...
  return {
    ...
    recommendation:
      capRate >= 6
        ? 'Good rental investment opportunity'
        : 'Below target cap rate - negotiate lower price',
  };
};
...
const issuesHandler: ToolHandler<IssuesInput, IssuesOutput> = async (input, ctx) => {
  ...
  const overallRisk = issues.some((i) => i.severity === 'critical' || i.severity === 'high')
    ? 'high'
